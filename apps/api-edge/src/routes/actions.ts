...
router.get('/:sessionId/resume', async (c) => {
  ...
  try {
    ...
    const attemptId = typeof session.metadata.capabilityExecutionAttemptId === 'string'
      ? session.metadata.capabilityExecutionAttemptId
      : null;
    if (attemptId && session.status === 'completed') {
      await resumeCapabilityExecutionAttempt(c.env, attemptId).catch((resumeErr) => {
        console.error('[actions] capability execution resume failed:', resumeErr instanceof Error ? resumeErr.message : String(resumeErr));
      });
    }
