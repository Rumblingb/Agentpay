 ...
 *
 * Connection strategy (matches the Render/Express backend's pattern):
 *   - Supabase pooled endpoint (port 6543) for runtime queries — use DATABASE_URL.
 *   - Hyperdrive: when the HYPERDRIVE binding is configured (production), its
 *     `.connectionString` replaces DATABASE_URL.  Hyperdrive keeps warm TCP
 *     connections to Supabase so each Worker invocation doesn't pay the full
 *     Supabase TLS handshake cost.
 *
