...
class IdentityVerifierAgent {
  ...
  async getIdentityRecord(agentId: string): Promise<{
    ...
    const activeCredentials = agent.verificationCredentials.filter(
      (c: any) => new Date() < new Date(c.expiresAt) && !c.revoked
    );
  ...
  private async signCredential(credential: VerificationCredential): Promise<string> {
    // Use HMAC (HS256) with the configured secret for local/dev signing.
    // In production you'd swap to an RSA keypair stored in a KMS and use RS256.
    const payload = this.serializeCredential(credential);
    return jwt.sign({ data: payload }, this.privateKey, { algorithm: 'HS256' });
  }

  private async verifySignature(token: string, payload: string): Promise<boolean> {
    try {
      const decoded: any = jwt.verify(token, this.privateKey, { algorithms: ['HS256'] });
      // Compare the signed payload to the expected serialized payload
      return decoded && decoded.data === payload;
    } catch (err) {
      return false;
    }
  }
