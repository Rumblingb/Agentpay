...
## Section 9: Protocol Integrations & Interoperability

### The Multi-Protocol Imperative

The agentic economy is not a monoculture. Different agent frameworks speak different payment dialects:
- Coinbase's **x402** standard uses HTTP 402 Payment Required headers
- IBM's **ACP** (Agent Communication Protocol) uses structured message payloads
- **AP2** (Agent Payment Protocol v2) adds cryptographic receipts and confirmation flows
- **Solana Pay** provides native on-chain transaction requests
- **Stripe** remains the fiat on-ramp of choice for agents with human owners

AgentPay solves the Babel problem with its **Protocol Abstraction Layer (PAL)**.
...
Incoming Request
  ...
  │  (headers → body → heuristics)  │
  └─────────────┬───────────────────┘
                │
      ┌─────────┼──────────┐────────────────┐
      ▼         ▼          ▼                ▼
   x402       ACP        AP2          Solana/Stripe
  Handler    Handler   Handler          Handler
      │         │          │                │
      └─────────┴──────────┴────────────────┘
                           │
                    AgentPay Ledger
               (AgentRank + Escrow + Audit)
```

Any agent can send a request to `/api/protocol/detect` to discover which protocol AgentPay detected from their request structure. This allows protocol-agnostic agents to self-configure.

### x402 — HTTP-Native Paywalls

The x402 standard extends HTTP with payment semantics. AgentPay's x402 middleware:

1. **Intercepts** requests to protected resources
2. **Returns 402** with a structured payment descriptor (amount, methods, AgentRank requirement)
3. **Accepts** payment proof in `X-AgentPay-Payment-Id` header on retry
4. **Forwards** verified requests to the resource handler

```typescript
// Protect any API route in one line
router.get('/premium-data', x402Paywall({ amountUsd: 100, minAgentRank: 400 }), handler);
```

### ACP — Agent Communication Protocol

ACP payment messages are structured JSON objects with sender/recipient DIDs. AgentPay exposes:
- `POST /api/acp/pay` — issue payment receipt
- `POST /api/acp/verify` — verify ACP payment token
- `GET /api/acp/schema` — machine-readable schema for agent discovery

### AP2 — Cryptographic Receipt Flow

AP2 adds a three-step payment flow with cryptographic receipts:
1. **Request** — payer initiates with task description + amount
2. **Receipt** — payee issues HMAC-signed receipt after task completion
3. **Confirm** — payer verifies receipt and releases funds

This prevents both non-payment and non-delivery, solving the fundamental two-sided trust problem in A2A commerce.

### Competitive Moat Through Protocol Coverage

By supporting all major protocols, AgentPay becomes the **neutral settlement layer** that any agent, framework, or marketplace can route through — regardless of which payment protocol they natively speak.

---

## Section 10: Agent Onboarding & Developer Experience

### The 5-Minute Promise

AgentPay is built to the "5-minute integration" standard: any developer should be able to go from zero to first transaction in under 5 minutes. The key pillars:

**1. One-command setup**
```bash
npx agentpay init
# Creates: .env, example.ts, registers a test agent, issues API key
```

**2. Framework-native tools**
Every major agent framework gets a pre-built, tested integration:
- CrewAI: `AgentPayTool` (drop-in `BaseTool` subclass)
- LangGraph: `agentPayNode` (drop-in state graph node)
- AutoGPT: Plugin file (copy to plugins directory)
- OpenAI: `agentpayTools` array (pass directly to `tools:` param)

**3. Protocol auto-detection**
Agents don't need to know which protocol to use. PAL detects it automatically.

**4. Hosted custodial wallets**
Agents with no Solana wallet get a hosted wallet with one API call — no key management required.

### Developer Journey

```
Developer finds AgentPay
         │
         ▼
    ...
    (real-time confirmations)
         │
         ▼
    Monitor AgentRank growth
    (reputation compounds over time)
```

### Mermaid: Full Escrow + Protocol Flow

```mermaid
sequenceDiagram
    participant PA as Payer Agent
    participant AP as AgentPay API
    participant PAL as Protocol Layer
    participant PB as Payee Agent
    participant OR as Behavioral Oracle

    PA->>AP: POST /api/escrow/create {payeeId, amount, task}
    AP->>OR: Check: is PA/PB flagged for fraud?
    OR-->>AP: clearance: OK
    AP-->>PA: {escrowId, status: "locked"}

    PA->>PB: Task assignment (via ACP/AP2/custom)

    PB->>PA: Work delivered + completionProof

    PA->>AP: POST /api/escrow/{id}/complete
    AP->>OR: Validate completion proof
    OR-->>AP: delivery: confirmed

    AP->>PAL: Select payment protocol (Solana/Stripe)
    PAL->>PB: Release funds
    PB-->>PAL: tx confirmed

    AP->>AP: Update AgentRank for PA (+10) and PB (+10)
    AP-->>PA: {status: "completed", txHash: "..."}
```

### SDK Quality Standards

Both the TypeScript and Python SDKs are built to production standards:
- **Full TypeScript types** — every request/response is typed
...
- **Publish-ready** — `scripts/publish-sdks.sh` automates npm + PyPI release
