...
// Lightweight rate-limits for public network endpoints (protects from scraping)
...
// In-memory cache for feed responses (short TTL)
let feedCache: { value: any; expiresAt: number } | null = null;
 ...
 */
router.get('/discover', publicLimiter, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/feed', publicLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // Return cached feed when available
    if (feedCache && Date.now() < feedCache.expiresAt && !req.query.refresh) {
      const cached = feedCache.value as any[];
      const slice = cached.slice(offset, offset + limit);
      res.json({ success: true, feed: slice });
      return;
    }
    ...
    feedCache = { value: transactions, expiresAt: Date.now() + 10_000 };
 ...
 */
router.get('/leaderboard', publicLimiter, async (req: Request, res: Response) => {
  try {
