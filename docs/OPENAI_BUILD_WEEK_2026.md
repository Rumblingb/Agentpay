...
1. the products satisfy the human's explicit category, merchant, budget, refund, delivery, stock, evidence, and approval rules
...
1. normalizes a human-owned Buyer Constitution and runs its deterministic category, merchant, budget, refund, delivery, freshness, and approval filters before any model call
...
The shopper sets category scope with compact visual controls and sees whether the result is `GPT-5.6 output verified` or `Policy-safe fallback`. Either path still requires exact human approval before the simulated, merchant-branded sandbox checkout review.
...
| Buyer Constitution follow-on | Required category and merchant scope, refundability and approval policy, 1.1 signed contracts, dashboard BFF migration, visual scope control, and adversarial policy tests |
...
A shopper begins with a visual need such as a rain-ready commute or small-space reset, then limits the agent to categories such as wear, home, or audio. AgentPay signs and enforces that Buyer Constitution across category, merchant, budget, refundability, delivery, stock, freshness, and approval before GPT-5.6 sees an opaque candidate. The model can reorder eligible evidence but cannot widen the market. The human approves the exact item and total before a simulated, merchant-branded sandbox checkout review.
...
| 0:15-0:35 | Change agent scope, budget, delivery, and returns | "This is not a shopping textbox. The human owns a visual constitution. AgentPay removes the wrong category or merchant, and anything over budget, late, stale, unavailable, or outside the return policy before AI runs." |
