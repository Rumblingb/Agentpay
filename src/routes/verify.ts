...
router.get('/:txHash', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const verified = result.rows.length > 0 && result.rows[0].status === 'confirmed';
    ...
    const secret = process.env.WEBHOOK_SECRET || process.env.AGENTPAY_HMAC_SECRET || 'default-secret';
