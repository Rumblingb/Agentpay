 ...
 * The returned sessionId is linked to a new transaction row via
 * stripe_payment_reference for webhook correlation.
 */
router.post('/fiat', authenticateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    // Create a transaction row to track this fiat payment
    ...
    await query(
      ...
      [
        ...
        paymentId,
        value.amountUsd,
        '', // no on-chain recipient for fiat payments
        'pending',
        0,
        0,
        expiresAt,
    ...
    logger.info('Fiat intent created', {
