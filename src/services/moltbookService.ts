...
import { encryptKeypair } from '../utils/walletEncryption.js';
...
export async function registerBot(
  ...
  // displayName is always index 2 in values array for tests
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : (err as any)?.message ?? String(err);
...
export async function getBotOverview(botId: string): Promise<Record<string, unknown> | null> {
  ...
  const [botRow, dailySpend, dailyEarnings, serviceIncome, subCount] = await Promise.all([
    query(
  ...
  const botRows = botRow?.rows ?? [];
  if (!botRows[0]) return null;
  ...
  const autoApprovedTodayRows = autoApprovedTodayRow?.rows ?? [];
  const autoApprovedToday = Number(autoApprovedTodayRows[0]?.total ?? 0);
...
export async function updateSpendingPolicy(
  ...
  await query(`UPDATE bots SET ${updates.join(', ')} WHERE id = $${idx}`, values);
...
export async function checkSpendingPolicy(
  ...
  const requirePinAbove =
    row.require_pin_above != null ? Number(row.require_pin_above) : null;
  ...
  const todaySpend = Number(todaySpendRow?.rows?.[0]?.total ?? 0);
  const todayAutoApproved = Number(todayAutoApproveRow?.rows?.[0]?.total ?? 0);

  if (todaySpend + amount > dailyMax) {
    const violation = {
      botId: internalId,
      amount,
    ...
    return {
      ...
      requiresPin: false,
      reason: 'Daily spending limit exceeded',
      remainingDaily: Math.max(0, dailyMax - todaySpend),
    };
  }
  ...
  return {
    ...
    requiresPin,
    remainingDaily: Math.max(0, dailyMax - todaySpend - amount),
    remainingAutoApproveDaily: Math.max(
      0,
      dailyAutoApproveCap - todayAutoApproved - (wouldBeAutoApproved ? amount : 0),
    ),
  };
}
...
export async function retrySubscription(
  ...
  if (sub.status === 'cancelled') {
    return {
      success: false,
      status: 'cancelled',
      reason: 'Cannot retry a cancelled subscription',
    };
  }
  ...
  logger.info('Subscription renewed', {
    subscriptionId,
    amount,
    subscriber: sub.subscriber_handle,
  });
...
export async function processSubscriptionRenewals(): Promise<{
  ...
  logger.info('Subscription renewal run completed', {
    processed: dueResult?.rows?.length ?? 0,
    renewed,
    failed,
  });

  return { processed: dueResult?.rows?.length ?? 0, renewed, failed };
}
 ...
 */
export async function getBotReputation(
  botId: string,
...
export async function getDailyStats(date?: string): Promise<unknown> {
  ...
  const result = await query(`SELECT * FROM moltbook_daily_stats WHERE date = $1`, [targetDate]);
